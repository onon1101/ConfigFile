...
exports.vlogDocSymbolProvider = void 0;
const vscode = require("vscode");
const core_1 = require("../core");
const util_1 = require("../util");
;
class VlogDocSymbolProvider {
    async provideDocumentSymbols(document, token) {
        // console.log('VlogDocSymbolProvider');
        const path = document.fileName;
        const vlogAll = await core_1.hdlSymbolStorage.getSymbol(path);
    ...
    makeDocumentSymbols(document, symbols) {
        ...
        for (const moduleSymbol of moduleSymbols) {
            ...
            const moduleDocSymbol = new vscode.DocumentSymbol(moduleName, moduleName, moduleKind, moduleRange, moduleRange);
            ...
            for (const symbol of symbols) {
                ...
                if (!((0, util_1.positionAfterEqual)(symbol.range.start, moduleSymbol.range.start) &&
                    (0, util_1.positionAfterEqual)(moduleSymbol.range.end, symbol.range.end))) {
                    continue;
                }
                if (!symbol.name) {
                    symbol.name = '???';
                ...
                if (symbol.type === 'parameter') {
                    if (!paramContainer.range) {
                        ...
                        paramContainer.docSymbol = new vscode.DocumentSymbol('param', 'param description', vscode.SymbolKind.Method, symbolRange, symbolRange);
                        moduleDocSymbol.children.push(paramContainer.docSymbol);
                    }
                    const paramDocSymbol = new vscode.DocumentSymbol(symbol.name, symbol.type, vscode.SymbolKind.Constant, symbolRange, symbolRange);
                    ...
                    if (!portContainer.range) {
                        ...
                        portContainer.docSymbol = new vscode.DocumentSymbol('port', 'port description', vscode.SymbolKind.Method, symbolRange, symbolRange);
                        moduleDocSymbol.children.push(portContainer.docSymbol);
                    }
                    const portDocSymbol = new vscode.DocumentSymbol(symbol.name, symbol.type, vscode.SymbolKind.Interface, symbolRange, symbolRange);
                    portContainer.docSymbol?.children.push(portDocSymbol);
                }
                else {
                    const symbolKind = this.getSymbolKind(symbol.type);
                    const symbolDocSymbol = new vscode.DocumentSymbol(symbol.name, symbol.type, symbolKind, symbolRange, symbolRange);
...
exports.vlogDocSymbolProvider = vlogDocSymbolProvider;
