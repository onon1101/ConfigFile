...
exports.vhdlLinterManager = void 0;
...
class VhdlLinterManager {
    constructor() {
        ...
        vscode.window.onDidChangeActiveTextEditor(editor => {
            ...
            if (hdlFs_1.hdlFile.isVhdlFile(currentFileName)) {
    ...
    async initialise() {
        ...
        for (const doc of vscode.workspace.textDocuments) {
            ...
            if (hdlFs_1.hdlFile.isVhdlFile(fileName)) {
                await this.lint(doc);
            }
        }
        global_1.LspOutput.report('<vhdl lsp manager> finish initialization of vhdl linter. Linter name: ' + this.activateLinterName, global_1.ReportType.Launch);
        // hide it if current window is not vhdl
        const editor = vscode.window.activeTextEditor;
        if (editor && hdlFs_1.hdlFile.isVhdlFile(editor.document.fileName)) {
            this.statusBarItem.show();
        }
        else {
            this.statusBarItem.hide();
    ...
    async updateLinter() {
        ...
        global_1.LspOutput.report(`<vhdl lsp manager> detect linter setting changes, switch from ${lastDiagnostorName} to ${diagnostorName}.`, global_1.ReportType.Launch);
        let launch = false;
    ...
    async activateVivado() {
        ...
        let launch = true;
        launch = await selectedLinter.initialise(enum_1.HdlLangID.Vhdl);
        if (launch) {
            this.statusBarItem.text = '$(getting-started-beginner) Linter(vivado)';
            global_1.LspOutput.report('<vhdl lsp manager> vivado linter has been activated', global_1.ReportType.Info);
        }
        else {
            ...
            global_1.LspOutput.report('<vhdl lsp manager> Fail to launch vivado linter', global_1.ReportType.Error);
        }
    ...
    async activateModelsim() {
        ...
        if (launch) {
            ...
            global_1.LspOutput.report('<vhdl lsp manager> modelsim linter has been activated', global_1.ReportType.Info);
        }
        else {
            this.statusBarItem.backgroundColor = new vscode.ThemeColor('statusBarItem.warningBackground');
            ...
            global_1.LspOutput.report('<vhdl lsp manager> Fail to launch modelsim linter', global_1.ReportType.Error);
        }
        this.currentLinter = selectedLinter;
    ...
    async activateDefault() {
        const selectedLinter = default_1.defaultVhdlLinter;
        let launch = true;
        if (launch) {
            this.statusBarItem.text = '$(getting-started-beginner) Linter(default)';
            global_1.LspOutput.report('<vhdl lsp manager> default build-in linter has been activated', global_1.ReportType.Info);
        }
        else {
            ...
            global_1.LspOutput.report('<vhdl lsp manager> Fail to launch default linter', global_1.ReportType.Error);
        }
        this.currentLinter = selectedLinter;
        this.activateLinterName = 'default';
        this.statusBarItem.show();
...
exports.vhdlLinterManager = vhdlLinterManager;
